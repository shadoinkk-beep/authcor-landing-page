...
const Navbar:FC = () => {
  ...
  //  window && window.addEventListener("scroll", changeNavbarColor);
    ...
    return <> 
    <header style={{zIndex:5}} className={`md:mx-auto text-gray-600 body-font fixed  w-full  ${pathname.split("/")[0] ==  "/" ? "dark:text-gray-400" : "bg-black/[0.5]" } ${colorChange
    ...
    <div className="container px-6 py-4 mx-auto md:flex md:justify-between items-center">
      ...
      <Link href={"/"} className="flex title-font font-medium items-center my-2 md:m-0 dark:text-white">
      ...
      {/* Mobile links */}
      <div  className={`${isMobNavOpen ? 'translate-x-0 opacity-100 ' : 'opacity-0 -translate-x-full '} absolute inset-x-0 z-20 w-full grid px-6 py-4 transition-all duration-300 ease-in-out bg-black/[0.9] md:mt-0 md:p-0 md:top-0 md:relative md:bg-transparent md:w-auto md:opacity-100 md:translate-x-0 md:flex md:items-center`}>
            <div className="flex flex-col md:flex-row md:mx-6 align-items text-white">
                ...
                <button onClick={()=>{setquotedisplay(true)}} className="relative justify-self-center m-2 w-[90%] rounded-full inline-flex md:hidden h-fit bg-red-500 flex-shrink-0 items-center justify-center overflow-hidden py-3 px-8 mx-8 font-medium font-semibold text-white shadow-2xl transition-all before:absolute before:h-0 before:w-0 before:rounded-md before:bg-white before:duration-100 before:ease-out hover:shadow-orange-600 hover:text-black hover:before:h-full hover:before:w-full">
      <span className="relative z-10">Get a quote</span>
    </button>
      </div>

      <button onClick={()=>{setquotedisplay(true)}} className="relative hidden md:inline-flex h-fit bg-red-500 rounded-lg flex-shrink-0 items-center justify-center overflow-hidden py-2 px-10 border-0 border-red-500 mx-8 font-medium font-semibold text-white shadow-2xl transition-all before:absolute before:h-0 before:w-0 before:rounded-md before:bg-white hover:text-black before:duration-100 before:ease-out hover:shadow-orange-600 hover:before:h-full hover:before:w-full">
      <span className="relative z-10">Get a quote</span>
    </button>
